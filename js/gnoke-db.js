/* ═══════════════════════════════════════════════════════════════
   ...
   FIX 7 — Added resetIDB() to the public API. close() released everything
            including _handle, which cooled the File System Access permission
            and forced Chrome to show an "Allow" prompt on every reconnect
            after a stale-DB recovery. resetIDB() closes and deletes only
            the IDB shadow (gnoke-db-shadow) while leaving _handle and the
            FS permission intact. open() can then re-run cleanly, rebuilding
            the IDB schema from scratch, without requiring a user gesture.
═══════════════════════════════════════════════════════════════ */
...
const GnokeDB = (() => {
  ...
  /* ── PUBLIC: resetIDB ────────────────────────────────────────
     Closes and deletes only the IDB shadow database.
     Leaves _handle and the File System Access permission warm.
     Use this to recover from a stale IDB schema without triggering
     Chrome's "Allow" permission prompt on reconnect.
     After resetIDB(), call open() again — it will rebuild the schema.
  ── */
  function resetIDB() {
    return new Promise((res, rej) => {
      if (_idb) { _idb.close(); _idb = null; }
      // _handle is intentionally preserved — FS permission stays warm
      _ready = false;
      const del = indexedDB.deleteDatabase(DB_NAME);
      del.onblocked = () => rej(new Error('IDB delete blocked — a connection is still open'));
      del.onsuccess = res;
      del.onerror   = () => rej(del.error);
    });
  }

  return { open, save, update, remove, query, drop, collections, configure, hasPendingWrites, close, resetIDB };
